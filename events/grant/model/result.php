...
while($list = $res->fetch()) {
	...
	$result['points'][$list['id']]["points"][$list['id_from']]["faculty"] = $list['faculty'];
...
foreach ($result['points'] as $id_project => $project) {
	...
	foreach ($result['points'][$id_project]["avg"] as $criteria => $value) {
		$result['points'][$id_project]['more_than_avg'] += (int)($value >= $result['points'][$id_project]["avg_all"]);
	}
