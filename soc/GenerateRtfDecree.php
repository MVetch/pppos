...
$mat_pod = $db->query("
    ...
    ON
        ...
        YEAR(mat_support.payday) = $year_needed ".($user->getLevel() == 2?"AND full_info.faculty = '".$fac_query['name']."'":"")."
");
...
$section->writeText('
    Председатель – '.$fac_query['fio_d'].' – (декан факультета директор института)
    Секретарь – Н.П. Шпица – заместитель председателя профбюро факультета
    Присутствовали:
    '.$fac_query['fio_z'].' – заместитель (декана факультета директора института) по внеучебной работе
    Е.А. Гурова –  председатель профбюро (факультета института)
    ...
    I. СЛУШАЛИ: '.$fac_query['fio_z_rp'].' – заместителя (декана факультета директора института) по внеучебной работе о рассмотрении заявлений об оказании материальной поддержки обучающимся.
    ...
    ВЫСТУПИЛИ: Заместитель (декана факультета директора института) '.$fac_query['fio_z_d'].'
...
$section->writeText('
    II. СЛУШАЛИ: '.$fac_query['fio_z_rp'].' – заместителя декана факультета (директора института) по внеучебной работе о рассмотрении заявлений студентов о назначении государственной социальной стипендии.
    ...
    ВЫСТУПИЛИ: Заместитель декана факультета (директора института) по внеучебной работе '.$fac_query['fio_z_rp'].'
